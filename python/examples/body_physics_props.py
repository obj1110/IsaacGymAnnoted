...
for i in range(num_envs):
    ...
    if i == 0:
        ...
        for dx in range(3):
            ...
            # 这句话是向仿真环境中添加一个新的演员（actor），即一个物体（在此情况下是一个立方体箱子），并将其句柄保存到 actor_handles 列表中
            # gymapi.Transform(p=gymapi.Vec3(dx, 0.1, 0.))：定义物体的初始位置和姿态
            actor_handles.append(
                # 定义所要创建对象的环境，定义环境中的对象，定义对象的初始位置，定义这个对象的名字，定义环境是第几个
                gym.create_actor(env, box_handles[0], gymapi.Transform(p=gymapi.Vec3(dx, 0.1, 0.)), 
                                 name, i, 0))

            # 这句话的作用是获取某个演员（actor）的刚体形状属性（rigid shape properties）。这些属性描述了物体在物理模拟中的行为特性，如摩擦力、恢复系数等。
        ...
        # 以上的代码，对移动的箱子，设置了其颜色，摩擦力和线速度
        # add target boxes to env
        # 下面是添加速度为0的箱子，也是指定其位置，指定其属性等等
    ...
    if i == 1:
        # add box actor
        # 定义gymapi.Transform对象，定义其初始位置和旋转角度等
        pose = gymapi.Transform()
        # 定义其在空中的位置，旋转的角度等等
        pose.p = gymapi.Vec3(0.0, 0.5, 0.0)
        pose.r = gymapi.Quat(0, 0, 0, 1)
        # create_actor就是将箱子添加到实际的环境中去
        box_handle = gym.create_actor(env, asset_box, pose, "actor1", i, 0)
        # 然后有一个handle
        # 问题就是这些handle具体都是被怎样使用的呢？到底是谁在调用这些handle？
        # 我在这个文件中没有找到调用的情况，我应该去其他位置去寻找
        actor_handles.append(box_handle)

        # 我反正不理解为什么要这样搞
        # 这段代码的功能是调整物体在物理仿真中的弹性和柔顺性，特别适用于希望模拟物体在碰撞时表现出弹性恢复和柔性形变的场景。例如，如果你想让物体在碰撞时既有弹性反弹又能有一定的形变，这种设置可以帮助你实现这一效果
